...
import { MV_STYLE_PRESETS, VIDEO_AI_PRESETS } from './presets';
...
export function generateChappyPrompt(
  ...
  const aiObj = VIDEO_AI_PRESETS.find((a) => a.id === settings.videoAI) || VIDEO_AI_PRESETS[0];
  ...
  return `あなたは世界最高峰のMVディレクターであり、同時に映像生成AI（${aiObj.name} 等）のプロンプトエンジニアです。
...
・使用予定の動画生成AI: ${aiObj.name}
・プロンプトスタイル指標: ${aiObj.promptStyle}
${customNote}
==================================================
...
5. **${aiObj.name}用 動画生成プロンプト (英語)** (AI動画生成ツールにそのまま入力できる高品質な英文プロンプト。被写体、背景、ライティング、カメラ動作を含めて記述)
